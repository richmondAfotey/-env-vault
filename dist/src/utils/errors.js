...
export class ProfileNotFoundError extends VaultError {
    constructor(profile) {
        super(`Profile '${profile}' does not exist. Create it with: ` +
...
export function red(text, useColor = process.stderr.isTTY) {
    return paint(text, ANSI_RED, useColor);
}
export function yellow(text, useColor = process.stderr.isTTY) {
    return paint(text, ANSI_YELLOW, useColor);
}
export function bold(text, useColor = process.stderr.isTTY) {
