...
/** Internal readline fields we poke; deliberately not part of the public API. */
...
function iface(): MutedReadline {
  ...
  return shared as MutedReadline;
}

function readLine(label: string, masked: boolean): Promise<string> {
  const rl = iface();
  // Swap readline's echo hook for this question.
  rl._writeToOutput = masked
    ? (s: string) => rl.output.write('*'.repeat(s.length))
    : (s: string) => rl.output.write(s);
